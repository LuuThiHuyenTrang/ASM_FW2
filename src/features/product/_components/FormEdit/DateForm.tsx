...
const DateForm = ({ data }: DateFormProps) => {
    ...
    const { form, onSubmit } = useProductMutation({
        ...
        onSuccess: () => {
            ...
            toast({
                variant: 'success',
                title: 'Chúc mừng bạn cập nhật ngày sinh của mèo thành công!',
                description: 'Cập nhật ngày sinh mèo thành công'
            })
        }
    })
    useEffect(() => {
    ...
    return (
        <div className='mt-6 border bg-slate-100 rounded-md p-4'>
            ...
            {!productEditStatus && <p className='text-sm mt-2'>{data?.date}</p>}
            {productEditStatus && (
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onHandleSubmit)} className='flex flex-col gap-y-8'>
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormControl>
                                        <Input {...field} placeholder='Nhập ngày sinh mèo' />
                                    </FormControl>
                                </FormItem>
                            )}
                        />
